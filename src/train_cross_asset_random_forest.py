...
MODEL_FIGURE_DIR = FIGURE_DIR / "random_forest"
...
def select_best_random_forest_model(X_train, y_train, X_val, y_val):
    """
    Try multiple Random Forest parameter settings.
    Select the best model using validation ROC-AUC.
    Does not plot or save tuning config results.
    """
    best_model = None
    best_params = None
    best_val_auc = -np.inf
    ...
    for idx, params in enumerate(RF_PARAM_GRID):
...
def plot_overfit_underfit_subplots(results_dict):
    ...
    plt.suptitle(
        "Cross-Asset Random Forest: Train vs Validation vs Test",
        fontsize=16,
    )
    plt.tight_layout(rect=[0, 0, 1, 0.95])

    save_path = MODEL_FIGURE_DIR / "random_forest_overfit_underfit_subplots.png"
...
def plot_overall_test_results_subplots(results_dict):
    ...
    plt.suptitle("Cross-Asset Random Forest: Overall Test Results", fontsize=16)
    plt.tight_layout(rect=[0, 0, 1, 0.95])

    save_path = MODEL_FIGURE_DIR / "random_forest_overall_test_results_subplots.png"
    plt.savefig(save_path, dpi=300)
    plt.close()
...
def plot_confusion_matrices_subplots(predictions_dict):
    ...
    plt.suptitle(
        "Cross-Asset Random Forest: Test Confusion Matrices",
        fontsize=16,
    )
    plt.tight_layout(rect=[0, 0, 1, 0.95])

    save_path = MODEL_FIGURE_DIR / "random_forest_confusion_matrices_subplots.png"
    plt.savefig(save_path, dpi=300)
    plt.close()
...
def plot_feature_importance_subplots(feature_importance_dict):
    ...
    plt.suptitle(
        "Cross-Asset Random Forest: Feature Importance by Stock",
        fontsize=16,
    )
    plt.tight_layout(rect=[0, 0, 1, 0.95])

    save_path = MODEL_FIGURE_DIR / "random_forest_feature_importance_subplots.png"
    plt.savefig(save_path, dpi=300)
    plt.close()
...
def main():
    ...
    for target_stock in TARGET_STOCKS:
        print("\n" + "=" * 70)
        print(f"Tuning Random Forest model for {target_stock}")
        print("=" * 70)
        ...
        best_model, best_params, best_val_auc, best_gap = select_best_random_forest_model(
            X_train,
            y_train,
            X_val,
            y_val,
    ...
    output_json = METRICS_DIR / "cross_asset_rf_metrics.json"
